    ...
    if(anciennePosition >= caseDebut && anciennePosition <= caseFin){
        ...
        for(int i = 0; i < tableauBool.size(); i++) {
            if ((tableauBool[i] == true) && (occupTerrain[i] == joueurIndex) && (typeHotel[i] == 1)) {
                ...
                std::vector<std::string> phrases2 = {"Vous n'etes pas elligible pour acheter des entrees pour les hotels que vous occupez."};
...
void jouerPartie(SDL_Renderer* renderer, Renderer& gameRenderer) {
    ...
    std::vector<int> prixLoyer = {100, 200, 250, 300};
    ...
    //initialisation aléatoire du joueur qui commence
    ...
    for (int tour = 1; tour <= 20; ++tour) {
        ...
        for (int ind = 0; ind < players.size(); ind++) {
            int i = players[ind]; 
            if (joueurs.getArgent(0) <= 0 || joueurs.getArgent(1) <= 0) {
                break;  // Fin du jeu si un des joueurs n'a plus d'argent
            }
            std::vector<std::string> joueurPhrases = {
                "C'est le tour de " + joueurs.getNom(i),
                    ...
                    if (event.type == SDL_MOUSEBUTTONDOWN) {
                        ...
                        if (mouseX >= 800 && mouseX <= 950 && mouseY >= 10 && mouseY <= 60) {
                            ...
                            std::vector<std::string> dePhrases = {
                                joueurs.getNom(i) + " avance de " + std::to_string(resultatDe) + " cases."
                            };
                            ...
                            std::vector<std::string> positionPhrases = {
                                joueurs.getNom(i) + " est maintenant a la case " + std::to_string(joueurs.getPosition(i))
                            };
                            ...
                            if (terrainAdjacent != nullptr) {
                                ...
                                if(numerocase == 12){
                                    ...
                                    if (occupTerrain[index] == i) {
                                        ...
                                        else if(entrees[index] == 0){
                                            prixEntree = prixLoyer[index] + 300;
                                        }
                                        else{
                                            prixEntree = prixLoyer[index];
                                        ...
                                        joueurs.ajouterArgent(deuxiemeJoueur, prixEntree);

                                        std::vector<std::string> phrasesTerrain = {
                                            "Vous devez payer le loyer de " + std::to_string(prixEntree) + " euros",
    ...
    std::vector<std::string> finPartiePhrases = {"Fin de la partie"};
    if(joueurs.getArgent(0) <= 0){
        finPartiePhrases = {"Alicia n'a plus d'argent", "Charbel a gagne la partie!!!"};
    }
    else if(joueurs.getArgent(1) <= 0){
        finPartiePhrases = {"Charbel n'a plus d'argent", "Alicia a gagne la partie!!!"};
    }
